...
Für R-Base plot () wandeln wir hierzu das `release_date` in einen Factor um. Damit die Levelreihenfolge stimmt, überführen wir die Daten in ein gesondertes Objekt. Für `ggplot()` muss die Häufigkeitstabelle zunächst in ein Datenframe überführt werden.
```{r}
#| label: LoesungAB00Ch
#| warning: false
#| layout-ncol: 2
# nimm nur unique Datumsangaben und sortiere diese
zeitstrahl <- sort(unique(ts$release_date))

# überführe in factor()
ts$release_date <- factor(ts$release_date,
                          ordered=TRUE,
                          levels = zeitstrahl)
# plotte mit Rbase
plot(table(ts$release_date), type="l",
     col="purple",
     ylab="Anzahl Tracks",
     xlab="Datum",
     main="Taylor Swift Alben")
# ggplot
# Häufigkeitstabelle der release_dates erstellen
tbl <- as.data.frame(table(ts$release_date))

# Plot mit ggplot2 erstellen
ggplot(tbl, aes(x = as.Date(Var1), y = Freq)) +
